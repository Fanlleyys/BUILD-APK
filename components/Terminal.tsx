...
export const Terminal: React.FC<TerminalProps> = ({ logs, status }) => {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto mt-6 md:mt-8 animate-in slide-in-from-bottom-10 duration-700">
      
      {/* === STATUS BAR & PROGRESS === */}
      <div className="mb-3 md:mb-4 flex items-center justify-between text-[10px] md:text-xs font-mono uppercase tracking-widest text-zinc-500">
        <span className="flex items-center gap-2">
            {status === BuildStatus.SUCCESS ? <CheckCircle2 className="text-emerald-500 w-3 h-3 md:w-4 md:h-4" /> : 
             status === BuildStatus.ERROR ? <AlertCircle className="text-red-500 w-3 h-3 md:w-4 md:h-4" /> :
             <Loader2 className="animate-spin w-3 h-3 md:w-4 md:h-4 text-brand-500" />}
      ...
      {/* Progress Bar Line */}
      <div className="h-1 w-full bg-zinc-800 rounded-full overflow-hidden mb-4 md:mb-6 relative">
        <div 
      ...
      {/* === TERMINAL WINDOW === */}
      <div className="rounded-xl overflow-hidden border border-zinc-800 bg-[#0c0c0c]/90 backdrop-blur-xl shadow-2xl shadow-black/50 font-mono text-xs md:text-sm relative group">
        
        {/* Terminal Header */}
        <div className="bg-zinc-900/80 border-b border-white/5 px-3 py-2 md:px-4 md:py-2 flex items-center justify-between select-none">
            {/* Kiri: Window Controls */}
            <div className="flex items-center gap-1.5 md:gap-2">
                <div className="w-2.5 h-2.5 md:w-3 md:h-3 rounded-full bg-red-500/80 hover:bg-red-500 transition-colors shadow-[0_0_8px_rgba(239,68,68,0.4)]" />
                <div className="w-2.5 h-2.5 md:w-3 md:h-3 rounded-full bg-yellow-500/80 hover:bg-yellow-500 transition-colors shadow-[0_0_8px_rgba(234,179,8,0.4)]" />
                <div className="w-2.5 h-2.5 md:w-3 md:h-3 rounded-full bg-green-500/80 hover:bg-green-500 transition-colors shadow-[0_0_8px_rgba(34,197,94,0.4)]" />
            </div>

            {/* Tengah: Title */}
            <div className="flex items-center gap-2 text-zinc-500 text-[10px] md:text-xs absolute left-1/2 -translate-x-1/2">
                <TerminalIcon className="w-3 h-3" />
                <span className="hidden sm:inline">build_server.exe — ssh root@builder-ai</span>
        ...
        {/* Terminal Body */}
        {/* Responsive Height: h-[300px] di HP, h-[400px] di Desktop */}
        <div className="p-3 md:p-4 h-[300px] md:h-[400px] overflow-y-auto space-y-1 
            scrollbar-thin 
