...
app.get('/', (req, res) => res.status(200).send('AppBuilder-AI v5.1 (Kotlin Fix) is Running. 🚀'));
...
// FIXED: Comprehensive Kotlin stdlib version resolution
function fixKotlinDependencies(androidRoot: string, kotlinVersion = '1.8.22'): boolean {
  ...
  try {
    ...
    // Find the dependencies block and add exclusions + force resolution
    const kotlinFixBlock = `
    // APPBUILDER_KOTLIN_FIX_APPLIED - Force consistent Kotlin stdlib versions
    configurations.all {
        resolutionStrategy {
...
`;
    ...
    // Insert before dependencies block or at the end
    if (content.includes('dependencies {')) {
      content = content.replace('dependencies {', `${kotlinFixBlock}\ndependencies {`);
    } else {
      content += `\n${kotlinFixBlock}\n`;
    }
...
// Additional fix: Update gradle.properties to force Kotlin version
function updateGradleProperties(androidRoot: string, kotlinVersion = '1.8.22'): boolean {
  ...
  try {
    ...
    // Check if already has kotlin version
    if (!content.includes('kotlin.version=')) {
      content += `\n# Added by AppBuilder-AI\nkotlin.version=${kotlinVersion}\n`;
...
`;
    ...
    // FIXED: Apply comprehensive Kotlin dependency fixes
    try {
      log('Applying Kotlin dependency resolution fixes...', 'info');
      
      const kotlinFixed = fixKotlinDependencies(androidDir, '1.8.22');
      const propsUpdated = updateGradleProperties(androidDir, '1.8.22');
      
      if (kotlinFixed && propsUpdated) {
        log('✅ Successfully applied Kotlin stdlib conflict resolution!', 'success');
      } else {
        log('⚠️ Partial Kotlin fix applied. Build may still have issues.', 'error');
      }
    } catch (e: any) {
    ...
    if (fs.existsSync(path.join(gradleDir, 'gradlew'))) {
      ...
      await runCommand('./gradlew', ['assembleDebug', '--refresh-dependencies'], gradleDir, log);
    } else {
